...
export function BookingForm() {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    setRequestUrl(url);
    window.dispatchEvent(new CustomEvent("clinic:intent", { detail: { action: "whatsapp_handoff", path: "/appointment" } }));
    window.open(url, "_blank", "noopener,noreferrer");
  };

  if (requestUrl) {
    return (
      <div role="status" tabIndex={-1} ref={(node) => node?.focus()} className="rounded-xl border border-green-200 bg-green-50 p-8 text-center">
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
      ...
      <p className="mb-6 text-sm text-gray-500">
        1. Complete your details → 2. Send in WhatsApp → 3. Wait for clinic confirmation.
      </p>
